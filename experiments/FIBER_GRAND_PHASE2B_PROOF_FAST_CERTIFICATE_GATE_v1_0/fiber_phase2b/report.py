...
def decide(repair: dict[str, Any], proof: dict[str, Any], replay: dict[str, Any], targeted: dict[str, Any] | None) -> dict[str, Any]:
    correctness = repair["status"] == "PASS" and proof["status"] == "PASS" and replay["status"] == "PASS"
    if not correctness:
    ...
    elif targeted is not None and targeted.get("conference_specialization_candidate"):
...
def make_report(output: Path, repair: dict[str, Any], proof: dict[str, Any], build: dict[str, Any], replay: dict[str, Any], targeted: dict[str, Any] | None) -> dict[str, Any]:
    decision = decide(repair, proof, replay, targeted)
    write_json(output / "SCIENTIFIC_DECISION.json", decision)
    target_text = "Not run: replay gate did not authorize new trials." if targeted is None else (
        f"Ran {targeted['trials']} trials in {targeted['cells']} cells; standard-gate cells={targeted['standard_gate_cells']}, "
        f"tail-gate cells={targeted['tail_gate_cells']}, specialization candidate={targeted['conference_specialization_candidate']}."
    )
    md = f"""# FIBER-GRAND Paper I — Phase 2B proof/fast-certificate gate
...
## Phase 2A artifact repair

- Status: **{repair['status']}**
- Original manifest entries: {repair['original_entries']}
- Line-ending-normalization mismatches diagnosed: {repair['crlf_normalization_count']}
- Unexplained mismatches: {repair['unexplained_mismatch_count']}
- Phase 2A was not rerun.

## Proof and exact arithmetic

- Status: **{proof['status']}**
- Exact proof-support checks: {proof['exact_cases']}
- The proof source now distinguishes arithmetic-operation complexity from bit complexity and handles the boundary cases explicitly.
...
"""
    (output / "PHASE2B_REPORT.md").write_text(md, encoding="utf-8", newline="\n")
    report={"created_utc":utc_now_iso(),"artifact_repair":repair,"proof":proof,"build":build,"replay":replay,"targeted":targeted,"decision":decision}
    write_json(output/"PHASE2B_REPORT.json",report)
    return report
