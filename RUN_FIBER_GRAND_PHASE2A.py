#!/usr/bin/env python3
"""One-command WSL launcher for the FIBER-GRAND Paper-I Phase-2A gate.

Default use from the connected VS Code WSL terminal:
    cd /home/afazeli2006/GRAND_Work
    python3 RUN_FIBER_GRAND_PHASE2A.py
...
PACKAGE_ZIP_BASENAME = "FIBER_GRAND_PHASE2A_BOUNDED_GATE_v1_0_2026-08-02.zip"
PACKAGE_ZIP_STEM = "FIBER_GRAND_PHASE2A_BOUNDED_GATE_v1_0_2026-08-02"
PACKAGE_ZIP_SHA256 = "59fda59ce3d78f3608bbefef67a55de7eeec44f40d56b5c886d10e640b63efa0"
PACKAGE_ROOT_NAME = "FIBER_GRAND_PHASE2A_BOUNDED_GATE_v1_0"
DEFAULT_REPO_ROOT = Path("/home/afazeli2006/GRAND_Work")
PACKAGE_RELATIVE_PATH = Path("experiments") / PACKAGE_ROOT_NAME
RESULTS_NAMESPACE = Path("results") / "FIBER_GRAND_Phase2A"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Install and run the FIBER-GRAND Phase-2A bounded gate in WSL.")
...
def add_local_git_excludes(git_dir: Path, logger: Logger) -> None:
    ...
    patterns = [
        ...
        "*.py[cod]",
        ".pytest_cache/",
        "results/FIBER_GRAND_Phase2A/**/FIBER_GRAND_PHASE2A_RETURN_*.zip",
    ]
...
def install_package(zip_path: Path, repo_root: Path, logger: Logger) -> Path:
    ...
    temporary_parent = Path(tempfile.mkdtemp(prefix=".fiber_phase2a_extract_", dir=str(repo_root)))
    try:
...
def prepare_venv(logger: Logger, stamp: str) -> Path:
    venv_root = Path.home() / ".venvs" / "fiber_grand_phase2a_v1_0"
    ...
    if not venv_python.exists():
        ...
        if rc != 0:
            raise RuntimeError("Failed to create the Phase-2A virtual environment")
    version = python_version(venv_python)
...
def git_commit_and_push(
    *,
    ...
    forbidden = [
        ...
        if path_has_venv(path) or Path(path).name.startswith("FIBER_GRAND_PHASE2A_RETURN_") and path.endswith(".zip")
    ]
    if forbidden:
    ...
    message = f"FIBER-GRAND Phase 2A bounded gate {status} {run_dir.name}"
...
def execute(args: argparse.Namespace, logger: Logger) -> None:
    ...
    atomic_json(
        run_dir / "WRAPPER_METADATA.json",
        {
            ...
            "profile": "smoke" if args.smoke else "bounded_default",
    ...
    if unit_rc != 0:
        ...
        config_name = "phase2a_smoke.json" if args.smoke else "phase2a_default.json"
        package_log = run_dir / "PACKAGE_CONSOLE.log"
        package_rc = run_streaming(
            [
                str(venv_python),
                str(package_target / "run_phase2a.py"),
    ...
    logger.write("Review the result directory and the Git commit when available before authorizing any larger campaign.")
...
def main() -> None:
    args = parse_args()
    temp_log = Path(tempfile.gettempdir()) / f"FIBER_GRAND_PHASE2A_WRAPPER_{utc_stamp()}_{os.getpid()}.log"
    logger = Logger(temp_log)
    try:
        logger.write("FIBER-GRAND Paper I — Phase 2A bounded-gate wrapper")
