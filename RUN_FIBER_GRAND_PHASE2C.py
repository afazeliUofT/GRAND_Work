#!/usr/bin/env python3
"""One-command WSL launcher for the FIBER-GRAND Paper-I Phase-2C gate.

Default use from the connected VS Code WSL terminal:
    cd /home/afazeli2006/GRAND_Work
    python3 RUN_FIBER_GRAND_PHASE2C.py
...
PACKAGE_ZIP_BASENAME = "FIBER_GRAND_PHASE2C_BASELINE_EXACT_DECODER_GATE_v1_0_2026-08-03.zip"
PACKAGE_ZIP_STEM = "FIBER_GRAND_PHASE2C_BASELINE_EXACT_DECODER_GATE_v1_0_2026-08-03"
PACKAGE_ZIP_SHA256 = "dedc6eb71748325edda2337a4ac34c4dd05f30c215f2de2457016fdbe073fadd"
PACKAGE_ROOT_NAME = "FIBER_GRAND_PHASE2C_BASELINE_EXACT_DECODER_GATE_v1_0"
DEFAULT_REPO_ROOT = Path("/home/afazeli2006/GRAND_Work")
PACKAGE_RELATIVE_PATH = Path("experiments") / PACKAGE_ROOT_NAME
RESULTS_NAMESPACE = Path("results") / "FIBER_GRAND_Phase2C"
EXPECTED_GITHUB_REPOSITORY = "afazeliuoft/grand_work"
MINIMUM_PYTHON = (3, 10)
MAX_UNCOMPRESSED_PACKAGE_BYTES = 100 * 1024 * 1024
REQUIRED_PHASE2B_R1_COMMIT = "7f8c87eb202dc7827439799ea7ed8e876dd4b73a"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Install and run the FIBER-GRAND Phase-2C baseline exact-decoder gate in WSL.")
...
def add_local_git_excludes(git_dir: Path, logger: Logger) -> None:
    ...
    patterns = [
        ...
        "*.py[cod]",
        ".pytest_cache/",
        "results/FIBER_GRAND_Phase2C/**/FIBER_GRAND_PHASE2C_RETURN_*.zip",
    ]
...
def install_package(zip_path: Path, repo_root: Path, logger: Logger) -> Path:
    ...
    temporary_parent = Path(tempfile.mkdtemp(prefix=".fiber_phase2c_extract_", dir=str(repo_root)))
    try:
...
def prepare_venv(logger: Logger, stamp: str) -> Path:
    venv_root = Path.home() / ".venvs" / "fiber_grand_phase2c_v1_0"
    ...
    if not venv_python.exists():
        ...
        if rc != 0:
            raise RuntimeError("Failed to create the Phase-2C virtual environment")
    version = python_version(venv_python)
...
def write_commit_ready_manifest(run_dir: Path) -> Path:
    """Hash the exact text artifacts intended for the Phase-2C result commit.
    ...
    scope_note.write_text(
        "# Phase 2C result-manifest scope\n\n"
    ...
    for path in sorted(candidate for candidate in run_dir.rglob("*") if candidate.is_file()):
        if path == manifest:
            continue
        if path.name.startswith("FIBER_GRAND_PHASE2C_RETURN_") and path.suffix == ".zip":
            continue
...
def git_commit_and_push(
    *,
    ...
    forbidden = [
        ...
        if path_has_venv(path) or Path(path).name.startswith("FIBER_GRAND_PHASE2C_RETURN_") and path.endswith(".zip")
    ]
    if forbidden:
    ...
    message = f"FIBER-GRAND Phase 2C baseline exact-decoder gate {status} {run_dir.name}"
...
def execute(args: argparse.Namespace, logger: Logger) -> None:
    ...
    ancestry = capture([
        ...
        REQUIRED_PHASE2B_R1_COMMIT, "HEAD",
    ])
    if ancestry.returncode != 0:
        raise RuntimeError(
            f"Required Phase 2B-R1 PASS commit {REQUIRED_PHASE2B_R1_COMMIT} is not an ancestor of the current HEAD"
        )
    logger.write(f"[repo] Phase 2B-R1 PASS commit verified as ancestor: {REQUIRED_PHASE2B_R1_COMMIT}")
    ...
    atomic_json(
        run_dir / "WRAPPER_METADATA.json",
        {
            ...
            "profile": "smoke" if args.smoke else "baseline_exact_decoder_default",
    ...
    if unit_rc != 0:
        ...
        config_name = "phase2c_smoke.json" if args.smoke else "phase2c_default.json"
        package_log = run_dir / "PACKAGE_CONSOLE.log"
        package_rc = run_streaming(
            [
                str(venv_python),
                str(package_target / "run_phase2c.py"),
                "--repo-root", str(repo_root),
                "--config", str(package_target / "config" / config_name),
                "--output", str(run_dir),
                "--cache-root", str(Path.home() / ".cache" / "fiber_grand_phase2c_v1_0"),
            ],
            logger=logger,
            cwd=package_target,
            env=env,
    ...
    logger.write(f"PHASE2C_DECISION={decision_label}")
...
def main() -> None:
    args = parse_args()
    temp_log = Path(tempfile.gettempdir()) / f"FIBER_GRAND_PHASE2C_WRAPPER_{utc_stamp()}_{os.getpid()}.log"
    logger = Logger(temp_log)
    try:
        logger.write("FIBER-GRAND Paper I — Phase 2C baseline exact-decoder gate wrapper")
